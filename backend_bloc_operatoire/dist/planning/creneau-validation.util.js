...
async function verifierCreneauValide(creneauRepo, date, heureDebut) {
    if (!date)
        return;
    ...
    const conflit = await creneauRepo.findOne({
        where: { date: date, heureDebut, statut: (0, typeorm_1.Not)(creneau_bloc_entity_1.StatutCreneau.ANNULE) },
    });
    if (conflit) {
