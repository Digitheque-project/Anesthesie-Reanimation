...
export class ChecklistApresOpService {
  ...
  async findOne(id: string) {
    ...
    if (!checklist) throw new NotFoundException(`Checklist après opération ${id} non trouvée`);
    const [enriched] = await this.accueilClient.enrichWithIdentity([checklist]);
    return enriched;
  }

  async update(id: string, dto: UpdateChecklistApresOpDto): Promise<ChecklistApresOp> {
    const checklist = await this.repo.findOne({ where: { id } });
    if (!checklist) throw new NotFoundException(`Checklist après opération ${id} non trouvée`);
    ...
    if (transfertVientDEtreConfirme) {
      await this.patientBlocStatutService.changerStatut(updated.patientId, PatientStatut.EN_SALLE_REVEIL);
    }
