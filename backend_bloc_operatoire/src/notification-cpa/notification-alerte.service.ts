...
export class NotificationAlerteService {
  ...
  async getAlertesUrgentes(): Promise<any> {
    ...
    const patientsUrgents = await this.patientBlocRepo.find({ where: { niveauUrgence: NiveauUrgence.URGENT } });
    ...
    for (const patient of patientsUrgentsEnrichis) {
      const creneau = await this.creneauRepo.findOne({ where: { patientId: patient.patientId, statut: StatutCreneau.PLANIFIE } });
      if (!creneau) {
    ...
    const notifsEnRetardRaw = await this.notifRepo.find({ where: { statut: StatutNotificationCPA.EN_ATTENTE, createdAt: LessThanOrEqual(dateLimite) }, relations: ['chirurgien'] });
  ...
  async getResumeJour(): Promise<any> {
    ...
    const [creneauxJourRaw, urgences, notifsEnAttente] = await Promise.all([
      ...
      this.patientBlocRepo.count({ where: { niveauUrgence: NiveauUrgence.URGENT } }),
