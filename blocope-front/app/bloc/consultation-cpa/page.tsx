...
const ANESTHESISTE_ID = '9c672c3d-ca54-4b49-b8e8-c67d50cc7cba'; // Dr. Bernard (Chirurgien)
  ...
  const handleValider = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        anesthesisteId: ANESTHESISTE_ID,
        ...
        taille: 170,
        poids: 70,
        examenCardiovasculaire: 'Normal',
        examenPulmonaire: 'Normal',
        examenNeurologique: 'Normal',
        colorationConjonctivale: 'Normale',
        abordVeineux: 'Bon',
        rachis: 'Normal',
        mallampati: scoreMallampati,
        ouvertureBuccale: 4,
        distanceMentoThyroidienne: 6,
        dents: 'Denture saine',
        tabac: 'Non fumeur',
        alcool: 'Occasionnel',
        scoreASA: typeof scoreASA === 'string' ? parseInt(scoreASA) : scoreASA,
        decision: 'APTE',
        ...
        jeune: 'À partir de minuit',
        preparationPhysique: 'Douche bétadinée',
        tachesInfirmieres: 'Constantes',
        dateVPA: dateVPA || undefined
      };

      console.log('📤 Envoi CPA:', payload);
      
      const response = await apiClient.post('/cpa', payload);
      console.log('✅ CPA créée:', response.data);

      if (dateVPA) {
        await planningService.reserverCreneau({
          patientId: patientIdFinal,
          chirurgienId: CHIRURGIEN_ID,
          date: dateVPA,
          heureDebut: heureVPA,
          heureFin: heureVPA,
          salle: 'VPA-1',
